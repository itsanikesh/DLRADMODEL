...
X_train0 = np.load('Xtest1.npy')
print X_train0.shape

Y_train00 = np.load('Ytest1.npy')
Y_train0 = Y_train00[:,:]
...
model = Sequential()
model.add(Dense(32, input_dim=3778, kernel_initializer='uniform', activation = 'sigmoid'))
model.add(Dense(32,                 kernel_initializer='uniform', activation = 'sigmoid'))
model.add(Dense(32,                 kernel_initializer='uniform', activation = 'sigmoid'))
model.add(Dense(1034,               kernel_initializer='uniform'))
# Compile model
model.compile(loss='mean_squared_error', optimizer='adam',metrics=['accuracy'])

#estimators = KerasRegressor(model, epochs=1, batch_size=4096)
model.fit(X_train,Y_train, epochs=400, batch_size=256, validation_split=0.1)
